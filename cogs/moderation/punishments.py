...
class ModCommands(commands.Cog):
    ...
    async def mute(
        self, ctx, member: discord.Member, duration: str = "28d",
        *, reason: str = "No reason provided.",
        ...
        if not ctx.guild.me.guild_permissions.moderate_members:
            return await ctx.send("I do not have the `moderate members` permission required for this.")

        if ctx.author.top_role.position <= member.top_role.position:
            return await ctx.send("You cannot mute a member with a higher role than you.")
        ...
        if match:
            ...
            if unit not in self.UNIT_CONVERTERS:
                return await ctx.send(f"Unknown duration unit `{unit}`. Use s, m, h, d or w.")
    ...
    async def ban(
        ...
        *, reason: str = "No reason provided."
        ...
        try: delete_message_days = int(days_str)
        except ValueError: reason = days_str + " " + reason

        if delete_message_days < 0: delete_message_days = 0
        elif delete_message_days > 7: delete_message_days = 7

        if not ctx.guild.me.guild_permissions.ban_members:
            return await ctx.send("I do not have the `ban members` permission required for this.")
    ...
    async def warn(
        ...
        *, reason: str = "No reason provided."
        ...
        if member.bot: return await ctx.send("You cannot warn a bot.", ephemeral=True)
        if member == ctx.author: return await ctx.send("You cannot warn yourself.", ephemeral=True)
        ...
        if not logger: return await ctx.send("Couldn't find logging command, warn failed.")
